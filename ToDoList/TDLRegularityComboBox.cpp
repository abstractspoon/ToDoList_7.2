...
TDI_REGULARITY CTDLRegularityComboBox::GetSelectedRegularity() const
{
	int nSel = GetCurSel();
	...
	// else
	return (TDI_REGULARITY)GetItemData(nSel);
}

int CTDLRegularityComboBox::SetSelectedRegularity(TDI_REGULARITY nRegularity)
{
	return CDialogHelper::SelectItemByData(*this, nRegularity);
}

CString CTDLRegularityComboBox::GetRegularity(TDI_REGULARITY nRegularity)
{
